...
export default class ProcessPayment {
    ...
    async execute (input: Input): Promise<Output> {
        ...
        await this.queue.publish("paymentProcessed", { rideId: input.rideId, transactionId: transaction.getTransactionId(), amount: transaction.amount, status: transaction.status });
        return {
